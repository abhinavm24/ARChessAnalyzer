...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate, EngineManagerDelegate {
    ...
    private var nextColor: String = "w"
    //var previewView: UIView!
    private var fen: [[String]] = Array(repeating: Array(repeating: " ", count: 8), count: 8)
    //private var fen = Array(repeating: Array(repeating: " ", count: 8), count: 8)
    ...
    private var banner0:UILabel!
    private var bannerLayer0: CALayer! = nil
    ...
    private var model: ChessPieceClassifier!
    ...
    private func displayBoard() {
        ...
        colorW.setFill()
        UIRectFill(rect)
        let moveStr:String! = "Best move: " + bestMoveNext!
        if(bestMoveNext != "") {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        bestMoveNext = ""
        gameFen="position fen "+fenStr+" "+nextColor+" "+castling+" -"
    ...
    private func changeLabel() {
        DispatchQueue.main.async {
            CATransaction.begin()
            self.bannerLayer0.removeFromSuperlayer()
            self.banner0.text = "Detecting Chessboard..."
            if(self.detectedChessBoard) {
                self.banner0.text = "Chessboard detected now segmenting..."
                self.detectedChessBoard = false
            } else if(self.segmentedChessBoard){
                self.banner0.text = "Chessboard segmented now analyzing..."
                self.segmentedChessBoard = false
            } else if(self.predictedChessBoard) {
                self.banner0.text = ""
                self.predictedChessBoard = false
            }
            self.rootLayer.addSublayer(self.bannerLayer0)
            CATransaction.commit()
        }
    ...
    override func viewDidLoad() {
        ...
        let bbounds = CGRect(x: 150, y: 550, width: 100, height: 30)
        
        button = UIButton(frame: bbounds)
        proceed = false
        ...
        let bounds: CGRect = CGRect(x:0,y:0,width:500, height: 50)
        let point = CGPoint(x: 175, y: 100)

        banner0 = UILabel(frame: bounds)
        banner0.font = UIFont.boldSystemFont(ofSize: 16.0)
        banner0.center = point
        banner0.textAlignment = .center
        banner0.textColor = .blue
        banner0.text = "Detecting Chessboard...."
        previewView.addSubview(banner0)
        bannerLayer0 = banner0.layer
    ...
    func translateMove(move: String) -> (Int,Int,Int,Int) {
        ...
        //print(tmpArry[0].unicodeScalars.map{$0.value})
        //print("a".unicodeScalars.map{$0.value}[0])
